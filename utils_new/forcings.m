...
T=7; % Temporal period.
alpha_T=5; % Number of temporal periods to represent.
dT=T/N; % Temporal resolution.
X=200; % Spatial period.
alpha_X=5; % Number of spatial periods to represent.
dX=X/N; % Spatial resolution.
...
df_X=1/(alpha_X*X);
% f_X=0:df_X:f_X_max;
f_X=-f_X_max:df_X:f_X_max;

% 2D range.
[FT,FX]=meshgrid(f_T,f_X);
...
spread=4;FT0=1/T;sigma_T=(FT0/3)/spread;FX0=1/X;sigma_X=(FX0/3)/spread; S=exp( -( (FT-FT0).^2/(2*sigma_T^2) + (FX-FX0).^2/(2*sigma_X^2) ) )+exp( -( (FT-FT0).^2/(2*sigma_T^2) + (FX+FX0).^2/(2*sigma_X^2) ) ); % Gaussians @(1/T,1/X) & (1/T, -1/X).

figure(1e5);
subplot(121);
surf(FT,FX,S,'edgecolor','interp','facecolor','interp'); xlim([0, 2/T]); ylim([-2/X, 2/X]); view([0,0,1]);

% Build phase.
PHASE_p_p=exp(1j*2*pi*rand(floor(length(f_X)/2)+1,length(f_T))); % Random Phase_(+, +).
...
[TT,TX]=meshgrid(t,x);
subplot(122);
surf(TT,TX,real(s),'edgecolor','interp','facecolor','interp');view([0,0,1]);axis([t(1),t(end),x(1),x(end)]);
