% THIS SCRIPT WAS REPLACED BY 'plot_time_v_dist.m'. PLEASE USE THIS ONE
% FROM NOW ON.
...
% rescale = - 1;
% while (rescale == - 1)
%   rescale = input(['[', mfilename, '] Rescale (0 for no, new value for yes)? > ']);
% end
% if (rescale ~= 0)
...
%   filter_data = input(['[', mfilename, '] Filter (0 for no, 1 for high-pass)? > ']);
% end
% if (filter_data ~= 0)
%   filter_fcutoff = - 1;
%   while (filter_fcutoff <= 0)
%     filter_fcutoff = input(['[', mfilename, '] Filter cutoff frequency? > ']);
%   end
%   switch filter_data
%     case 1
%       for(i = 1:nstat)
...
%       data_v(i, :) = data_v_HP;
%   end
%   disp(['[', mfilename, ', WARNING] Data was high-pass filtered, with cutoff frequency ', num2str(filter_fcutoff), '.']);
%   clear('data_v_HP');
%  otherwise
...
%   normalise = input(['[', mfilename, '] Normalise data? (0 for no, 1 for yes) > ']);
% end
% 
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % Treatment.                  %
...
%   scale = input(['[', mfilename, '] Rechoose coefficient (0 for no, new value for yes)? > ']);
% end
% 
...
%   labeleach = input(['[', mfilename, '] Label each line? (0 for no, 1 for yes) > ']);
% end
% if (labeleach == 1)
%   % YTicks.
...
%     scale = input(['[', mfilename, '] Draw scale examples? (0 for no, 1 for yes)? > ']);
%   end
%   if (scale == 1)
