...
!  builds the global mass matrix
  ...
  use specfem_par, only: myrank,any_elastic,any_acoustic,any_gravitoacoustic,any_poroelastic, &
                                ...
                                ! MODIF DG
                                rmass_inverse_acoustic_DG, ibool_DG, &
                                ! MODIF NS
                                rmass_inverse_NS
  ...
  rmass_inverse_NS = 0._CUSTOM_REAL
        ...
        if (PML_BOUNDARY_CONDITIONS) then
          ...
          if (this_element_has_PML) then
            ...
            if (AXISYM) then
              ...
              ! NS MODIF
              rmass_inverse_NS(iglob) = rmass_inverse_NS(iglob) &
                   + wxgll(i)*wzgll(j)*jacobian(i,j,ispec)
                   
              !WRITE(*,*) "-------> ", iglob, i, j, ispec, rmass_inverse_NS(iglob) 
...
! (this can happen when some degrees of freedom have been removed from some of the global arrays)
  ...
  if (any_acoustic) then
    ...
    where(rmass_inverse_NS <= 0._CUSTOM_REAL) rmass_inverse_NS = 1._CUSTOM_REAL
  endif
  if (any_gravitoacoustic) then
...
! compute the inverse of the mass matrix
  ...
  ! MODIF NS
  rmass_inverse_NS = 1._CUSTOM_REAL / rmass_inverse_NS
