...
! number of layers in the model
  integer, parameter :: NR_LAYER = 2500
...
! region flag to assign the Atmosphere model
  ...
  double precision :: x,z,frac,tmp1,tmp2,tmp3,tmp4, gamma_temp_prev, gamma_temp
...
! read all the values in the 1D model once and for all
  ...
  ! tmp3 = Scale height
  ! tmp4 = kappa
  read(10,*) z_atmos(i),density_atmos(i),tmp1,vp_atmos(i),p_atmos(i),Htab(i),gravity_atmos(i),Nsq_atmos(i),tmp4,&
                mu_atmos(i),eta_atmos(i), wx_atmos(i), wz_atmos(i)
...
! loop on all the elements of the mesh, and inside each element loop on all the GLL points
  ...
  if (ii == 1) then
    ...
    gamma_temp = (vp_atmos(1)**2)/(gravity_atmos(1)*Htab(1))
...
! interpolate from radius_ak135(ii-1) to r using the values at ii-1 and ii
    ...
    gamma_temp = (vp_atmos(ii)**2)/(gravity_atmos(ii)*Htab(ii))
    gammaext(ibool_DG(i,j,ispec)) = gamma_temp_prev + frac * (gamma_temp-gamma_temp_prev)
