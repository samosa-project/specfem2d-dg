...
function [figureHandle, tight_axes] = ABC_plot_NRJs(OFDIRS_struct, DISPLAYNAMES_struct, LS_strct, colours_runs, outputfigpath)
  ...
  gap_h = 0.05; gap_w = 0.05;
  marg_h_low = 0.10; marg_h_upp = 0.06;
  marg_w_lef = 0.09; marg_w_rig = 0.01;
...
%       yticklabels({});
  ...
  set(tight_axes([1,4]),'xlim',[0,.25],'xtick',[-1:.05:10]);
  set(tight_axes([2,3,5,6]),'xlim',[0,.6],'xtick',[-1:.1:10]);
  set(tight_axes(1),'ylim',[-1,1]+[-1,1]*.1);
  set(tight_axes(2),'ylim',[0,1]+[-1,1]*.1);
  set(tight_axes(3),'ylim',[-1,1]+[-1,1]*.1);
  set(tight_axes(4),'ylim',[0,1]+[-1,1]*.1);
  set(tight_axes(5),'ylim',[0,1]+[-1,1]*.1);
  set(tight_axes(6),'ylim',[-19e4,1]+[-1e4,1]*.1);
...
%   set(tight_axes(1:3), 'ylim', [-1, 1.15], 'yscale', get(tight_axes(1), 'yscale'));
%   set(tight_axes(4:6), 'ylim', [-1.5, 1.15], 'yscale', get(tight_axes(1), 'yscale'));
  ...
  for i=1:numel(ll);set(ll(i), 'Position', get(ll(i), 'Position')+[.03,-.32,0,0]);end
