...
! starts reading in parameters from input Database file
  ...
  read(IIN,"(a80)") datlin
  read(IIN,*) dynamic_viscosity_cte_DG
  
  read(IIN,"(a80)") datlin
  read(IIN,*) thermal_conductivity_cte_DG
  
  read(IIN,"(a80)") datlin
  read(IIN,*) tau_eps_cte_DG
  
  read(IIN,"(a80)") datlin
  read(IIN,*) tau_sig_cte_DG
...
! reads in interface dimensions
  ...
  if (ninterface > 0) then
    ...
    allocate(inum_interfaces_acoustic_DG(ninterface))
    ...
    ! MODIF DG
    allocate(ibool_interfaces_acoustic_DG(1,1))
    allocate(nibool_interfaces_acoustic_DG(1))
    allocate(inum_interfaces_acoustic_DG(1))
...
!
  ...
  subroutine set_simulation_domains()
  ...
  end subroutine set_simulation_domains
