...
!-----------------------------------------------
  ...
  use specfem_par, only: nglob_DG,ninterface_acoustic_DG, &
                         inum_interfaces_acoustic_DG, &
  ...
  do iinterface = 1, ninterface_acoustic_DG
    ...
    !WRITE(*,*) ">>>>>>>>>>>>><", iinterface, num_interface
    ...
    do ipoin = 1, nibool_interfaces_acoustic_DG(num_interface)
      ...
      ! copies array values to buffer
      buffer_send_faces_vector_DG(ipoin,iinterface) = array_val1(iglob)
      
    enddo

  enddo
  
  !buffer_send_faces_vector_DG_temp = buffer_send_faces_vector_DG
  
  !WRITE(*,*) "nb_values : ", nb_values, cpttt
  ...
  do iinterface = 1, ninterface_acoustic_DG
    ...
    nb_values = nibool_interfaces_acoustic_DG(num_interface)!*2

    ! non-blocking send
    call MPI_ISEND( buffer_send_faces_vector_DG(1,iinterface), &
             nb_values, CUSTOM_MPI_TYPE, &
             my_neighbours(num_interface), 12, MPI_COMM_WORLD, &
    ...
    ! starts a non-blocking receive
    call MPI_IRECV ( buffer_recv_faces_vector_DG(1,iinterface), &
             nb_values, CUSTOM_MPI_TYPE, &
             my_neighbours(num_interface), 12, MPI_COMM_WORLD, &
             tab_requests_send_recv_DG(ninterface_acoustic_DG+iinterface), ier)
     
     !WRITE(*,*) "TAAAAAAAAA", iinterface, (ninterface_acoustic_DG+iinterface), &
     !   tab_requests_send_recv_DG(ninterface_acoustic_DG+iinterface), &
     !   tab_requests_send_recv_DG(iinterface)
...
!-----------------------------------------------
  ...
  do iinterface = 1, ninterface_elastic
     ...
     do i = 1, nibool_interfaces_elastic(num_interface)
        ...
        !if(buffer_recv_faces_vector_el(ipoin+1,iinterface) > 0) WRITE(*,*) myrank,"TEST", &
        !        buffer_recv_faces_vector_el(ipoin+1,iinterface)
        ipoin = ipoin + 2
     enddo
