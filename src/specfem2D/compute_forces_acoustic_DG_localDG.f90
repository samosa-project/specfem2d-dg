...
! compute forces in the acoustic elements in forward simulation and in adjoint simulation in adjoint inversion
  ...
  use specfem_par, only: nglob_DG,nspec,ispec_is_acoustic, &
                         ...
                         MPI_transfer, i_stage, p_DG_init, gammaext_DG, muext, etaext, T_init!, diag_MPI!, &
  ...
  if(timelocal == 0) then
        p_DG_init = p_DG
        T_init = (E_DG/rho_DG - 0.5*((rhovx_DG/rho_DG)**2 + (rhovz_DG/rho_DG)**2))/(5/2)
  endif
  ...
  call compute_viscous_tensors(T_viscous_DG, V_viscous_DG)
      ...
      do j = 1,NGLLZ
        ...
        !- (gamma_euler - ONE)*rho_DG(iglob)*potential_dphi_dz_DG(ibool(i,j,ispec))*coord(2, ibool(i,j,ispec))
          ...
          do k = 1,NGLLX
                ...
                !dT_dxi    = dux_dxi    + T(ibool_DG(k,j,ispec)) * real(hprime_xx(i,k), kind=CUSTOM_REAL)
                !dT_dgamma = dux_dgamma + T(ibool_DG(i,k,ispec)) * real(hprime_zz(j,k), kind=CUSTOM_REAL)
          enddo
          ...
          !!!!!!!!!!!!!
          ! Heat flux
          !temp_unknown  = coef_thermal(i,j,ispec)*dT_dx
          !temp_unknown2 = coef_thermal(i,j,ispec)*dT_dz
...
!
    ...
    do  i = 1,NGLLX
      ...
      do while (j <= NGLLZ)
        ...
        elseif(ispec_is_acoustic_coupling_el(i,j,ispec,3) >= 0) then
               ...
               tangential_v = 0!veloc_x_DG(iglobM)*tx + veloc_z_DG(iglobM)*tz
              ...
              ! p_DG_P = p_DG(iglobM)-(elastic_tensor(i_el,j_el,ispec_el,1)*nx+elastic_tensor(i_el,j_el,ispec_el,4)*nz)!!1.!
               ...
               !WRITE(*,*) "p_DG_init(iglobM)", p_DG_init(iglobM), p_DG(iglobM)
               
               !p_DG_P = p_DG_init(iglobM)!p_DG(iglobM)
               ...
               !veloc_x_DG_P = 0!veloc_x_DG(iglobM)
               !veloc_z_DG_P = 0!-veloc_z_DG(iglobM)!-0.2*((timelocal-0.1)/0.1)*(1/0.1)*exp(-((timelocal-0.1)/0.1)**2)!
               !
               !p_DG_P = p_DG(iglobM)!p_DG_init(iglobM)!p_DG(iglobM)
               
               
               !WRITE(*,*) "COOL>>>",p_DG_init(iglobM)
               
        !       WRITE(*,*) it, "COOL>>>",(gamma_euler - 1.)*( E_DG(iglobM) &
        !- (0.5)*rho_DG(iglobM)*( veloc_x_DG(iglobM)**2 + veloc_z_DG(iglobM)**2 ) ), p_DG_init(iglobM)
               !p_DG_init(iglobM) = p_DG_P
        ...
        duz_dx = ZERO
        duz_dz = ZERO
        if(muext(i,j,ispec) > 0 .OR. etaext(i,j,ispec) > 0) then
        ...
        dot_E(iglobM)     = dot_E(iglobM) &
                + weight*( veloc_x_DG(iglobM) * temp_unknown + veloc_z_DG(iglobM) * temp_unknown2 )*nz
        ...
        !if(it == 26) &
        !WRITE(*,*) "WEIGHT >> ", dot_E(iglobM), weight, flux_n, lambda, jump, HALF
        dot_E(iglobM) = dot_E(iglobM) - weight*(flux_n + lambda*jump)*HALF
        
        !!!!!!!!!!!!!
        ! Heat flux
        !temp_unknown_M  = coef_thermal(i,j,ispec)*dT_dx
        !temp_unknown_P  = coef_thermal(i,j,ispec)*dT_dx
                
        !temp_unknown2_M = coef_thermal(i,j,ispec)*dT_dz
        !temp_unknown2_P = coef_thermal(i,j,ispec)*dT_dz
  ...
  !      maxval(dot_E), minval(dot_E), maxval(dot_rho), minval(dot_rho)
   ...
   subroutine compute_viscous_tensors(T_DG, V_DG, rho_DG, rhovx_DG, rhovz_DG, E_DG, veloc_x_DG, veloc_z_DG)
...
! compute forces in the acoustic elements in forward simulation and in adjoint simulation in adjoint inversion
  ...
  use specfem_par, only: nglob_DG,nspec,ispec_is_acoustic, &
                         ...
                         myrank, NPROC, ninterface_acoustic
  ...
  real(kind=CUSTOM_REAL) :: temp_Tx, temp_Tz, temp_Vxx, temp_Vxz, temp_Vxz, temp_Vzz
  
  ! Local
  real(kind=CUSTOM_REAL), dimension(nglob_DG) :: &
         rho_DG, rhovx_DG, rhovz_DG, E_DG, veloc_x_DG, veloc_z_DG, V_DG, T_DG
    ...
    if (ispec_is_acoustic(ispec)) then
      ...
      do j = 1,NGLLZ
        do i = 1,NGLLX
          ...
          temp_Vxz = duz_dx * jacobianl
          temp_Vzz = duz_dz * jacobianl
          
          ! along x direction and z direction
          ! and assemble the contributions
          grad_Tx(iglob)  = dot_Tx(iglob)  + temp_Tx * wxl * wzl
          grad_Tz(iglob)  = dot_Tz(iglob)  + temp_Tz * wxl * wzl
          grad_Vxx(iglob) = dot_Vxx(iglob) + temp_Vxx * wxl * wzl
          grad_Vxz(iglob) = dot_Vxz(iglob) + temp_Vxz * wxl * wzl
          grad_Vzx(iglob) = dot_Vzx(iglob) + temp_Vzx * wxl * wzl
          grad_Vzz(iglob) = dot_Vzz(iglob) + temp_Vzz * wxl * wzl
...
! compute forces in the acoustic elements in forward simulation and in adjoint simulation in adjoint inversion
  ...
  use specfem_par, only: ibool_before_perio, ibool_DG, coord, MODEL, &
        rhoext, windxext, pext_DG, gravityext, gammaext_DG, Htabext_DG,&
        etaext, muext, coord_interface
  ...
  veloc_z_DG_P = 0.!windzext(i,j,ispec)
   ...
   !etaext(i,j,ispec)  = 0
   muext(i,j,ispec)  = muext(1,1,1)
   endif
   
  !!!!!!!!!!!!!!!!!!!!!!!!!!
   ! MSISE-like wind profile
   !veloc_x_DG_P = -2*exp(-((z - 45)/12)**2) &
   !+ 3.5*exp(-((z - 65)/14)**2) &
   !- 75*exp(-((z - 110)/3)**2) &
   !+ 120*exp(-((z - 120)/3)**2) &
   !+ 115*exp(-((z - 142)/10)**2)
   !
   !if(z<=240) &
   !veloc_x_DG_P = veloc_x_DG_P + 75*exp(-((z - 240)/50)**2)
   !if(z>240) &
   !veloc_x_DG_P = veloc_x_DG_P + (95*exp(-((z - 240)/60)**2) - 20)
  
  !WRITE(*,*) x,z,"rho_DG_P", rho_DG_P, p_DG_P, gammaext_DG(ibool_DG(i,j,ispec)), gravityext(i,j,ispec)
  ...
  !      p_DG_P       = rho_DG_P**gamma_euler
   ...
   !theta    = theta0*exp(N*N*z/G)!theta0/exner
   !p_DG_P   = P0*exner**(cp/R)
   !rho_DG_P = (exner**(cnu/R))*P0/(R*theta)
   !if(z > 0) &
   !veloc_x_DG_P = 00
  ...
  subroutine SlopeLimitLin(ulimit, ispec, ul, v0, vkm1_x,vkp1_x,vkm1_z,vkp1_z)
    ...
    implicit none 
       ...
       ! stop
        ...
        call minmod_computeB(ulimit_temp, minmod, 3, 1,hx)
       ...
       !WRITE(*,*) "ulimit_tempx", ulimit_temp
        ...
        call minmod_computeB(ulimit_temp, minmod, 3, 1,hz)
        !call minmod_compute(ulimit_temp, minmod, 3, 1,ispec)
        !WRITE(*,*) "uz", uz
  ...
  subroutine minmod_computeB(R, v, n, m, h)
