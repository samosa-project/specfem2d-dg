...
!
  ...
  subroutine compute_kernels_ac_DG()
...
! see e.g. Tromp et al. (2005)
  ...
  use specfem_par, only: nspec,ispec_is_acoustic,ibool,ibool_DG,deltat, &
                         ...
                         gamma_ac_DG_kl, c_ac_DG_kl, v0_ac_DG_kl
 ...
 ! use specfem_par_gpu,only: Mesh_pointer,deltatf
  ...
  !local variables
  integer :: i,j,k,ispec,iglob,iglob_DG
  
  double precision :: gammal, rho0l, p0l, v0l, gravityl, c_adiab_l, E0_l
  ...
  double precision :: b_drhovx_dxl, b_drhovx_dzl, b_drhovz_dxl, b_drhovz_dzl, b_dE_dxl, &
        ...
        b_rhovx_l, b_rhovz_l, b_E_l, b_rho_l, rhovx_l, dtrho_l, &
        rhovz_l, E_l, rho_l, vx_l, vz_l, p_l 

  double precision, dimension(2) :: Sigma_dv0_x, Sigma_dv0_z
    ...
    do ispec = 1,nspec
      if (ispec_is_acoustic(ispec)) then
        do j = 1, NGLLZ
          ...
          E0_l = p0l/(gammal-1.) + (rho0l/2.)*v0l**2
          ...
          dtrho_l = dot_rho(iglob_DG)
          rhovz_l = rhovz_DG(iglob_DG)
          E_l     = E_DG(iglob_DG)
          rho_l   = rho_DG(iglob_DG)
          vx_l = rhovx_l/rho_l
          vz_l = rhovz_l/rho_l
          p_l     = (gammal-1)*(E_l - (1./2.)*( (rhovx_l/rho_l)**2 + (rhovz_l/rho_l)**2 ))
            
            ! Heat capacity kernel
            gamma_ac_DG_kl(i,j,ispec) = gamma_ac_DG_kl(i,j,ispec) - ( &
                (E_l + (rho0l/2.)*(2*v0l**2 + 2*v0l*vx_l) + (rho_l/2.)*(v0l**2))*v0l*b_dE_dxl &
                + (rho0l/2.)*(v0l**2)*(vx_l*b_dE_dxl + vz_l*b_dE_dzl) &
                + (E_l + (rho0l/2.)*(2*v0l**2 + 2*v0l*vx_l) + (rho_l/2.)*(v0l**2))*Div_b_rhov &
            ) * deltat
            
            ! p0/rho0 known => sound velocity kernel from heat capacity kernel
            c_ac_DG_kl(i,j,ispec) = gamma_ac_DG_kl(i,j,ispec)/(2*c_adiab_l*rho0l/p0l)
            
            Sigma_dv0_x(1) = (rho0l+rho_l)*2*v0l + 2*vx_l*rho0l
            Sigma_dv0_x(2) = rho0l*vz_l
            Sigma_dv0_z(1) = rho0l*vz_l
            Sigma_dv0_z(2) = 0
            
            ! Horizontal wind velocity kernel
            v0_ac_DG_kl(i,j,ispec)    = v0_ac_DG_kl(i,j,ispec) - ( &
                rho_l*b_drho_dxl + (gammal-.1)*((rho_l + rho0l)*v0l + rho0l*vx_l)*v0l*b_dE_dxl &
                + (gammal - 1.)**rho0l*v0l*(vx_l*b_dE_dxl + vz_l*b_dE_dzl) &
                + (E0_l + E_l + (gammal - 1.)*( E_l + ((rho_l+rho0l)/2.)*( v0l**2 + 2*v0l*vx_l ) ))*b_dE_dxl &
                + (gammal-1.)*rho0l*(v0l + vx_l)*Div_b_rhov &
                + (Sigma_dv0_x(1)*b_drhovx_dxl + Sigma_dv0_x(2)*b_drhovx_dzl) &
                + (Sigma_dv0_z(1)*b_drhovz_dxl + Sigma_dv0_z(2)*b_drhovz_dzl) &
                - dtrho_l*rhovx_l &
                 ) * deltat
  ...
  end subroutine compute_kernels_ac_DG
