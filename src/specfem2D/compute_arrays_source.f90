...
! ------------------------------------------------------------------------------------------------------
  ...
  use specfem_par,only: myrank,NSTEP,P_SV, &
                        ...
                        ispec_selected_rec,seismotype, deltat
  ...
  if (seismotype == 1 .or. seismotype == 2 .or. seismotype == 3) then
    ...
    do icomp = 3,3
      ...
      do itime = 1, NSTEP
        ...
        junk = (itime-1)*deltat
        !if(adj_src_s(itime,icomp) > 0) WRITE(*,*) trim(filename),"JUNK : ", junk, adj_src_s(itime,icomp)
        perio = 1
        to    = 11
        adj_src_s(itime,icomp) = (&
                  - (2d0/(perio/4d0))*((junk-(to-perio/4d0))/(perio/4d0))* &
                           (exp(-((junk-(to-perio/4d0))/(perio/4d0))**2)) &
                  + (2d0/(perio/4d0))*((junk-(to+perio/4d0))/(perio/4d0))* &
                           (exp(-((junk-(to+perio/4d0))/(perio/4d0))**2)) ) 
        to    = 2                  
        adj_src_s(itime,icomp) = adj_src_s(itime,icomp) + (&
                  - (2d0/(perio/4d0))*((junk-(to-perio/4d0))/(perio/4d0))* &
                           (exp(-((junk-(to-perio/4d0))/(perio/4d0))**2)) &
                  + (2d0/(perio/4d0))*((junk-(to+perio/4d0))/(perio/4d0))* &
                           (exp(-((junk-(to+perio/4d0))/(perio/4d0))**2)) ) 
  ...
  do k = 1, NGLLZ
    do i = 1, NGLLX
      if (P_SV) then
        ...
        !WRITE(*,*) "MAXVAL: ", maxval(adj_src_s(:,3)), maxval(adj_src_s(:,1))
      else
        ! SH-case
