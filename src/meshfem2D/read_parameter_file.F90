...
! reads in DATA/Par_file
  ...
  call read_value_double_precision_p(MINMOD_FACTOR, 'solver.MINMOD_FACTOR')
  if (err_occurred() /= 0) stop 'error reading parameter MINMOD_FACTOR in Par_file'
  ...
  call read_value_double_precision_p(dynamic_viscosity, 'solver.dynamic_viscosity')
  if (err_occurred() /= 0) stop 'error reading parameter dynamic_viscosity in Par_file'
  
  call read_value_double_precision_p(thermal_conductivity, 'solver.thermal_conductivity')
  if (err_occurred() /= 0) stop 'error reading parameter thermal_conductivity in Par_file'
  
  call read_value_double_precision_p(tau_epsilon, 'solver.tau_epsilon')
  if (err_occurred() /= 0) stop 'error reading parameter tau_epsilon in Par_file'
  
  call read_value_double_precision_p(tau_sigma, 'solver.tau_sigma')
  if (err_occurred() /= 0) stop 'error reading parameter tau_sigma in Par_file'
  
  call read_value_double_precision_p(constant_p, 'solver.constant_p')
  if (err_occurred() /= 0) stop 'error reading parameter constant_p in Par_file'
