...
function [RHS_q1, RHS_q21, RHS_q22, RHS_q3] = compute_RHS(it, DX, q1, q21, q22, q3, forward)
  ...
  % Constants.
  global g_z mu lambda kappa gamma_atmos
  
  % Source.
  global isource jsource VSTF
  
  % Linearised Euler/NS: q1=rho', q21=rho_0v'_x, q22=rho_0v'_y, q3=E'. define more practical quantities.
  global include_diffusive_terms % Import switch. If 0, linearised Euler. If 1, linearised Navier-Stokes.
  %%%%%%%%%%%%%%%%%%%%%%%
  % For convective      %
  % terms (\Sigma^c).   %
  %%%%%%%%%%%%%%%%%%%%%%%
  global RHO0 V0X V0Y E0 P0 % Import initial fields.
...
%   P0=(c.^2).*squeeze(initial_state(1,:,:))./gamma_atmos; % Initial pressure (ideal gas hypothesis). % THIS IS MOVEABLE OUTSIDE TIME LOOP.
  ...
  % Derivatives.        %
  %%%%%%%%%%%%%%%%%%%%%%%
  % Import derivatives of initial fields.
  global DXV0X DXV0Y DYV0X DYV0Y DIVV0
  
  % Compute $\partial_xv_{0,x}$ and $\partial_xv_{0,y}$.
...
%     gammal_x(1,:,1) = gamma_atmos(1:NX,j);
    ...
    % Linearised Euler/NS: q1=rho', q21=rho_0v'_x, q22=rho_0v'_y, q3=E'.
    %%%%%%%%%%%%%%%%%%%%%%%
    % Convective terms    %
    % (\Sigma^c).         %
    %%%%%%%%%%%%%%%%%%%%%%%
    % "-" here because convective terms are classicaly expressed in the LHS.
...
%     Flux(4,:,j) = rhocsq*dvx(1:NX,j) + rho*dvy(1:NX,j)*g;
    ...
    % (\Sigma^d).         %
    %%%%%%%%%%%%%%%%%%%%%%%
    % "+" since diffusive terms are classicaly in the RHS.
    if(include_diffusive_terms)
...
%     Deriv_x_E     = deriv(DELTAX, size_flux, Flux(4,:,j), forward);
    ...
    % Update RHS by adding component.
    ...
    % Transpose is only needed in Matlab, for compatibility of dimensions.
  end
...
%     Flux(4,i,:) = rhocsq*dvy(i,1:NY) + rho*dvy(i,1:NY)*g;
    ...
    % "+" since diffusive terms are classicaly in the RHS.
    if(include_diffusive_terms)
      flux(2,i,:) = flux(2,i,:) + permute(mu*(DXdvy(i,1:NY) + DYdvx(i,1:NY)), [3,1,2]       ); % First component of $\Sigma_v(v')_{2,:}$.
      flux(3,i,:) = flux(3,i,:) + permute(2*mu*DYdvy(i,1:NY) + lambda*DIVdv(i,1:NY), [3,1,2]); % Second component of $\Sigma_v(v')_{2,:}$.
      flux(4,i,:) = flux(4,i,:) + permute(DIFFUSIVE_TENSOR_ENERGY_Y(i,1:NY), [3,1,2]        );
      % "permute" is only needed in Matlab, for compatibility of dimensions.
    end
...
%   RHS_q1 = RHS_q1; % Always zero.
  ...
  RHS_q22 = RHS_q22 + q1*g_z;
  % Energy: only $\rho_0g_yv'_z$ if $g_x=0$ and initial velocity is fully horizontal ($v_{0,y}=0$).
%   RHS_q3 = RHS_q3 + g_y*(q22 + q1.*V0Y); % $g_z(\rho_0v'_z + \rho'v_{0,y})$.
  RHS_q3 = RHS_q3 + g_z*q22;
