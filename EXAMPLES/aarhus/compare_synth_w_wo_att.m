...
OFD_w = [cFldr,filesep,'OUTPUT_FILES_402141_2090_lns_allatt']; OFD_wo = [cFldr,filesep,'OUTPUT_FILES_402830_2090_lns_noatt'];
% OFD_w = [cFldr, filesep, 'OUTPUT_FILES_402036_2090_fns_allatt']; OFD_wo = [cFldr,filesep,'OUTPUT_FILES_402879_2090_fns_noatt'];
...
op.Lower = [0, fits_amp{i-1}.b, -Inf]; op.Upper = [Inf, fits_amp{i-1}.b, Inf]; fits_amp{i} = fit(Dsy', Afp_w, fitfunc_amp, op); colfit{i}=Cw; i=i+1; % fit without to get b
% plot nrg
ms = 250;
...
%   dnam = ['$\alpha=', scientific_latex_notation(fits{i}.a,2,0), '$, $\beta=',sprintf('%.2f', fits{i}.b),'$, $c_1=',sprintf('%.2f', fits{i}.c),'$'];
  dnam = ['$\alpha=', scientific_latex_notation(fits_nrg{i}.a,2,0), '$, $\beta=',sprintf('%.2f', fits_nrg{i}.b),'$'];
