...
toppanel_z = 0.3; marg_left = 0.08; marg_right = 0.05; leftpanel_h=0.3; gaph = 0.1; gapz=0.13; gapzradpat = 0.01; margbot=0.11;
tightAxesPField = tight_subplot(1, 1, [0, 0], [1-toppanel_z, 0.01], [marg_left, marg_right]);
tightAxesSpectraDefault = tight_subplot(1, 1, [0, 0], [margbot, toppanel_z+gapz], [marg_left, 1-leftpanel_h]);
...
CMAP = colormaps_custom([-1,-blk_pfield, -(1+thrsh_pfield)/2, -thrsh_pfield, 0, thrsh_pfield, (1+thrsh_pfield)/2, blk_pfield,1], [[0,0,1].*[0.1,0.75,1]';[0.9,0.9,1];[1,1,1];[1,0.9,0.9];[1,0,0].*[1,0.75,0.1]'], 0);
set(tightAxesPField, 'CLim', pfield_clim, 'Colormap', CMAP, 'XLim', XLIM/1e3, 'YLim', YLIM/1e3);
set(tightAxesPField, 'dataaspectratio', [1,0.7,1]);

axes(tightAxesSpectraDefault);
pow = -1; fac = 1e-3;
absfftname = ['\left|\widehat{P}\left(k_x,k_z\right)\right|'];
CBYLAB_fft = ['radiated infrasound, $\log_{10}\left(',absfftname,'\right)$'];
...
CMAP = flipud(CMAP);
set(tightAxesSpectraDefault, 'xscale', 'log', 'yscale', 'log', 'xlim', XLIMfft*fac, 'ylim', YLIMfft*fac, 'xtick', fft_xtick, 'ytick', fft_ytick, 'xticklabel', fftxtl, 'yticklabel', fftytl);
xlabel(XLAB_fft); ylabel(YLAB_fft);
...
ylabel({'radiated', 'infrasound'});
xlabel(['$\leftarrow$ towards Spain $|$ launch angle [deg] $|$ towards France $\rightarrow$']);
...
set([tightAxesSpectraDefault, tightAxesRadPat(1)], 'colormap', CMAP, 'clim', fft_clim);
