...
% LTOPO_over_L0 = 0.33; OF = 'OUTPUT_FILES_4214696_fns';
...
switch(LTOPO_over_L0)
  ...
  case 0.33
    ...
    TIT_addendum = 'with $L_0/3$ topography';
  case 3
...
title(TIT);
customSaveFig(fig_field, [figfieldpath], {'png', 'eps'}, 9999);
...
customSaveFig(fig_spect, [figspectpath], {'png', 'eps'}, 9999);
...
if(not(LTOPO_over_L0==Inf))
  ...
  if(exist(basefile,'file'))
    ...
    caxis([-maxneg,maxpos]);
    xlabel(XLAB); ylabel(YLAB); title(TIT);
    customSaveFig(fig_spectdiff, [figspectdiffpath], {'png', 'eps'}, 9999);
  else
